...
namespace FoW
{
    ...
    public class FogOfWarChunkManager : MonoBehaviour
    {
        ...
        Vector3Int CalculateBestChunk(Vector3 pos)
        {
            Vector3Int chunk = new Vector3Int(
                Mathf.RoundToInt(pos.x / (_fogOfWar.mapSize / 2)) - 1,
                Mathf.RoundToInt(pos.y / (_fogOfWar.mapSize / 2)) - 1,
        ...
        void ForceLoad()
        {
            ...
            Vector3Int desiredchunk = CalculateBestChunk(_followPosition);

            // move fow
            float chunksize = _fogOfWar.mapSize * 0.5f;
            _fogOfWar.mapOffset = new Vector2(desiredchunk.x, desiredchunk.y) * chunksize + Vector2.one * (chunksize);
