...
namespace FoW
{
    ...
    public class FogOfWarUnit : MonoBehaviour
    {
        ...
        bool[] CalculateLineOfSightCells(FogOfWarTeam fow, FogOfWarPhysics physicsmode, Vector3 eyepos, float distance, out int visiblecellswidth)
        {
            ...
            Vector2 playerworldpos = FogOfWarConversion.WorldToFogPlane(_transform.position, fow.plane);
        ...
        void FillShape(FogOfWarTeam fow, FogOfWarShape shape)
        {
            if (antiFlicker)
            {
                ...
                shape.eyePosition = FogOfWarConversion.SnapWorldPositionToNearestFogPixel(fow, FogOfWarConversion.WorldToFogPlane(_transform.position, fow.plane));
